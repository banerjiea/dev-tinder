...
const FOOTER_ALLOWED_PATHS = ['/login', '/privacy-policy', '/terms-of-service', '/refund-policy', '/contact-us'];
 
const Footer = () => {
  const location = useLocation();
  const isLoginPage = location.pathname === '/login';
  const showFooter = FOOTER_ALLOWED_PATHS.includes(location.pathname);
      ...
      <footer className="footer sm:footer-horizontal bg-neutral text-neutral-content items-center p-4 fixed bottom-0">
        ...
        {isLoginPage ? (
          <nav className="grid-flow-col gap-4 md:place-self-center md:justify-self-end text-sm">
            ...
            <Link to="/refund-policy" className="hover:underline hover:text-white transition-colors">
              Refund Policy
            </Link>
            <Link to="/contact-us" className="hover:underline hover:text-white transition-colors">
              Contact Us
            </Link>
          </nav>
        ) : (
